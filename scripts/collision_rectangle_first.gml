#define scr_collision_rectangle_first
///scr_collision_rectangle_first(x1,y1,x2,y2,object,prec,notme)
//
//  Returns the instance id of an object colliding with a given rectangle and
...
//
{
    ...
    sx = dx - ox;
    sy = dy - oy;
    inst = collision_rectangle(ox,oy,dx,dy,object,prec,notme);
    if (inst != noone) {
        while ((abs(sx) >= 1) || (abs(sy) >= 1)) {
            sx /= 2;
            sy /= 2;
            i = collision_rectangle(ox,oy,dx,dy,object,prec,notme);
            if (i) {
                dx -= sx;
                dy -= sy;
                inst = i;
